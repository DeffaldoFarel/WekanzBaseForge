...
export default function AuthSettingsPage() {
  ...
  const handleCopyUserId = (id: string) => {
    ...
    setCopiedId(id);
    toastSuccess(`User ID '${id}' copied to clipboard`);
    setTimeout(() => {
                          ...
                          {authUsers.map((u) => (
                            <tr key={u.id} className="hover:bg-secondary/30 transition-colors">
                              <td className="p-3 pl-4 font-mono text-xs">
                                <div className="inline-flex items-center gap-1.5 bg-secondary/80 border border-border px-2 py-0.5 rounded text-foreground font-medium group">
                                  <span className="select-all">{u.id}</span>
                                  <button
                                    type="button"
                                    onClick={() => handleCopyUserId(u.id)}
                                    className="text-muted-foreground hover:text-foreground opacity-60 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-accent cursor-pointer"
                                    title="Copy User ID"
                                  >
                                    {copiedId === u.id ? (
                              ...
                              <td className="p-3 font-mono text-xs text-foreground font-medium">
                                {u.email}
                              </td>
